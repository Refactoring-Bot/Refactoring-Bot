 ...
 */
@Component public class GithubDataGrabber {

		@Autowired ObjectMapper mapper;
		@Autowired BotConfiguration botConfig;
		...
		public GithubRepository checkRepository(String repoName, String repoOwner, String botToken, String apiLink)
				...
				if (apiLink == null || apiLink.isEmpty()) {
						configUri = createURIFromApiLink(
								GITHUB_DEFAULT_APILINK + "/repos/" + repoOwner + "/" + repoName);
				} else {
		 ...
		 * @throws Exception
		 */
		public void checkGithubUser(String botUsername, String botToken, String botEmail, String apiLink)
				throws Exception {
		...
		public GithubPullRequest createRequest(GithubCreateRequest request, GitConfiguration gitConfig)
				...
				try {
						// Send request to the GitHub-API
						return rest
								.exchange(pullsUri, HttpMethod.POST, new HttpEntity<>(request), GithubPullRequest.class)
								.getBody();
				} catch (RestClientException r) {
